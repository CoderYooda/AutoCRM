...
class WarrantController extends Controller
{
    public static function warrantDialog($request)
    {
        ...
        $refund_summ = $request['ostatok'] != $request['itogo'] ? $request['ostatok'] : $request['itogo'] ;
...
//        }
    ...
    public static function getWarrants(Request $request)
    {
        ...
        $warrants = Warrant::select(DB::raw('
            ...
            ->when($request['isIncoming'] !== null, function ($query) use ($request) {
                $query->where('warrants.isIncoming', (bool)$request['isIncoming']);
            })
        ...
        foreach ($warrants as $warrant){
            $warrant->date = Carbon::parse($warrant->date)->format('d.m.Y H:i');
        }
