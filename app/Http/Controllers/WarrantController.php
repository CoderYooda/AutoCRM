    ...
    public static function getWarrants($request)
    {
        ...
        $warrants =
            Warrant::select(DB::raw('
                warrants.created_at, warrants.created_at as date, warrants.id as id, IF(warrants.isIncoming = 1, "Приходный ордер","Расходный ордер") as type, IF(partners.type = 0, partners.fio,partners.companyName) as partner, dds_articles.name as dds, cashboxes.name as cashbox, warrants.summ
            '))
