...
class EmployeeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->expectsJson() && $request['search'] == NULL){
            $content = view(get_template() . '.employee.index', compact('request'))->render();
        ...
        {
            $content = view(get_template() . '.employee.elements.list_container', compact('request'))->render();
            return response()->json([
                'html' => $content,
                'target' => 'ajax-table-employee',
            ], 200);
        } else {
            return view(get_template() . '.employee.index', compact('request'));
        }
    }
