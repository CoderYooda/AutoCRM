<form id="provider_order_form" action="">
    ...
    <div data-simplebar style="max-height: 400px;">
        @foreach($providerorder->products()->get() as $product)
            @php $count = $product->pivot->count - $providerorder->getProductEntredCount($product->id) @endphp
            <div class="providerorder_article_elem list-item inblocked mini-list-element pointer" data-id="{{  $product->id }}">
                <div class="inblock">
                    ...
                    <div class="w-200 list-body b-r pr-2 compressed" style="max-height: 38px;overflow: hidden;flex: 2;">
                        <label class="md-check h-20 pr-2">
                            <input class="checked_field" id="article_provider_order_{{ $product->id }}" name="products[{{  $product->id }}][iclude]" type="checkbox" @if($count <= 0) @else checked @endif>
                            <i class="success"></i>
                        </label>
                    ...
                    <div class="list-body" style="margin-top: -5px;margin-bottom: -5px; flex: 0;">
                        <input class="form-control form-control-sm count_item" style="width: 75px" type="number" step="1" name="products[{{  $product->id }}][count]" value="{{ $count }}">
