    ...
    <div class="m-15">
        <div class="box mb-15">
            <div class="p-15 d-flex">
                <div>
                    {{--<span class="system_balance pr-15">Баланс: <b>{{ number_format($company->balance, 2, ',', ' ')  }} <sup>₽</sup></b></span> |--}}

                    @if($company->blocked)
                        <span class="system_balance"> Компания "{{ $company->name }}" заблокирована. Для активации свяжитесь с администратором</span>
                    @elseif($company->payed_days < Carbon\Carbon::now()->timestamp)
                        <span class="system_balance"> Компания "{{ $company->name }}" заблокирована. Для активации выберите тариф</span>
                    @else
                        <span class="system_balance pr-15">Состояние компании: <b>Активно</b>. До отключения: <b>{{ $company->getPayedDays() }} дн.</b></span>
                    @endif
