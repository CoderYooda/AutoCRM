<div class="tab-pane p-3 active" id="{{$class}}_tab_base">
    ...
    <div class="form-group">
        @foreach($stores as $store)
            ...
            <div class="input-group mb-3">
                <input type="number" min="0" name="storage[{{ $store->id }}][retail_price]" onchange="{{ $class }}.changeRetailPrice(this, {{ $store->id }});" class="form-control ml-0" placeholder="Розничная цена" value="@if($product){{ $product->stores->find($store->id)->pivot->retail_price ?? '0' }}@endif" >
            </div>
        @endforeach
    </div>
</div>
