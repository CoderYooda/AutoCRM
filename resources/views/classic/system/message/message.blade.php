<div class="box-color {{ $message->type }} aside_message w-350 mb-10 p-10">
    <button class="btn_close" onclick="window.systemMessages.readMessage(this, {{ $message->id }})">×</button>
    ...
    <div class="box-body">
        ...
        @if( $message->kind && $message->kind_id)
            ...
            @endphp
            <p onclick="systemMessages.modal.hide();setTimeout(function() { openDialog( '{{ $dialogName }}', '{{ $dialogParams }}' ) }, 200)" class="m-0 linkable">{{ $message->message }}</p>
        @else
