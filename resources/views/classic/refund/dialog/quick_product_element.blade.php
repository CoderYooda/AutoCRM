<tr class="product_list_elem" id="product_selected_{{ $product->id }}_new">
    ...
    <td class="text-right">
        @if(isset($request) && $request['refer'] != null)
            <button onclick="{{ $request['refer'] }}.removeItem('{{ $product->id }}', 'new')" type="button" class="trash-button"><i class="fa fa-trash"></i></button>
        @else
            <button onclick="{{ $class }}.removeItem('{{ $product->id }}', 'new')" type="button" class="trash-button"><i class="fa fa-trash"></i></button>
        @endif
    </td>
</tr>
